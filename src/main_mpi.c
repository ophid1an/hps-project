...
static void print_results(const struct result *res, uint8_t p, uint8_t b,
    uint8_t run, uint8_t fixed_tasks, uint8_t current_n_tasks, FILE *fptr, uint8_t first_call);
...
int main(int argc, char *argv[])
{
    ...
    if (taskid == 0) {
        ...
        // If fixed_tasks is set to 0 apply algorithms using
        // 1 up to numtasks tasks
        static uint8_t fixed_tasks = 0;
        uint8_t max_tasks = numtasks;
        ...
        while ((c = getopt(argc, argv, "p:b:u:r:t:")) != -1)
            ...
            case 'r':
                ...
                break;
            case 't':
                fixed_tasks = strtoul(optarg, NULL, 10);
                break;
            default:
                return EXIT_FAILURE;
            }
        ...
        size_t buf_length = (1UL << 19) * u / sizeof(uint32_t);
        ...
        // Find approximation of distinct items with HyperLogLog++ using MPI
        printf("\nHyperLogLog++ using MPI\n");
        ...
        for (uint8_t run = 1; run <= runs; ++run) {
            uint8_t current_n_tasks = 1;
            if (fixed_tasks != 0) {
                current_n_tasks = fixed_tasks;
                max_tasks = fixed_tasks;
            }
            for (; current_n_tasks <= max_tasks; ++current_n_tasks) {
                ...
                for (size_t i = 0; i < chunks; ++i) {
                    ...
                    // (only when needed)
                    if (chunks != 1 || (run == 1 && (current_n_tasks == 1 || fixed_tasks != 0))) {
                    ...
                    res.estimate = hllpp_mpi(buf, chunk_size, end_of_buffer, b);
                ...
                print_results(&res, p, b, run, fixed_tasks, current_n_tasks, fptr, first_call_to_print);
    ...
    uint8_t run, uint8_t fixed_tasks, uint8_t current_n_tasks, FILE *fptr, uint8_t first_call)
{
    ...
    // If number of tasks is specified, don't calculate and print
